...
#            imgs=np.expand_dims(imgs,axis=0)
        ...
        self.imgs=imgs
        self.msks=msks
        fmsks = msks > 0 # Compute foreground masks by including all single cell masks
        self.fmsks = fmsks
        self.timeList=timeList
        self.imgfileList=imgfileList

    def get_nucChanImgs(self, n_frame): #get nuclear channel images
        self.n_frame=n_frame
        nF=self.nF
        nucImgs=[None]*nF
        e_images=np.zeros(nF)
        for iF in range(self.nF):
            ...
            try:
                ...
                if e:
                    e_images[iF]=1
                    dsetName = "/images/img_%d/nuc" % int(n_frame)
                    dset=dataIn[dsetName]
                    nucImgs[iF]=dset[:]
                dataIn.close()
            except:
    ...
    def get_imageSet_trans(self):
        ...
        stack_inds=np.unique(self.imgfileSet).astype(int)
        for istack in stack_inds:
            sys.stdout.write('registering cell masks '+self.fileList[istack]+'\n')
            inds=np.where(self.imgfileSet==istack)
            inds=inds[0]
    ...
    def replace_garbage_image_data(self, frameID, ncellsLastFrame, ncellsCurrFrame):
        ...
        if diffCells > 50:
          ...
          self.mskSet[frameID, :, :] = self.mskSet[frameID-1, :, :]
          self.fmskSet[frameID, :, :] = self.fmskSet[frameID-1, :, :]
    ...
    def save_all(self):
        ...
        pickle.dump(self,objFileHandler,protocol=protocol_Pickle)
...
#        self.Xtraj=self.Xd[:,0:neigen]
    ...
    #### Transition Matrix function if clustering is performed using deeptime
    def get_transitionMatrixDeeptime(self, x0, x1, n_clusters):
        
        # Assuming x0 and x1 already contain the assigned cluster indices for each data point
        indc0 = x0
        indc1 = x1
        
        Cm = np.zeros((n_clusters, n_clusters))
        for itt in range(len(indc0)):
            Cm[indc0[itt], indc1[itt]] += 1
