...
class cellPoseTraj():
    ...
    def initialize(self, fileSpecifier, modelName):
        ...
        self.cmskchannel = None # None for single-channel masks, or chosen channel for multi-channel masks
    ...
    def get_image_data(self, n_frame):
        ...
        self.n_frame = n_frame # Number of frames within an image file
        nF = self.nF
        timeList = np.array([])
    ...
    def get_cell_blocks(self, msks, minsize = 1):
        ...
        for ic in range(ncells):
            indc = np.where(msks == ic)
            if not indc[0].size < minsize: #==0:
                idGood = np.append(idGood, ic)
                for idim in range(imgDim):
                    cellblocks[ic, idim, 0] = np.min(indc[idim])
                    cellblocks[ic, idim, 1] = np.max(indc[idim]) + 1
            else:
    ...
    def get_imageSet_trans_turboreg(self):
        ...
        for istack in stack_inds:
            ...
            inds = inds[0]
            msk0 = self.cmskSet[inds, :, :]
            msk0 = np.abs(msk0) > 0
            sr = StackReg(StackReg.TRANSLATION)
            tmats = sr.register_stack(msk0, reference='previous')
    ...
    def get_cell_images(self, indcells=None):
        ...
        for ic in indcells:
            ...
            nx = np.shape(mskcell)[0]
            ny = np.shape(mskcell)[1]
    ...
    def get_lineage_bunch_overlap(self, distcut=45.0, distcutb=300.0, overlapcut=10.0, cellcut=10.0, 
        ...
        if self.visual:
            plt.figure(figsize = (10, 8))
            nx = self.cmskSet.shape[1]; ny = self.cmskSet.shape[2]
            maxdx = np.max(nx - self.cmskSet_t[:,1]); mindx = np.min(0 - self.cmskSet_t[:,1]);
            maxdy = np.max(ny - self.cmskSet_t[:,2]); mindy = np.min(0 - self.cmskSet_t[:,2]);
            if pathto is None:
                pass
            else:
    ...
    def get_lineage_btrack(self, distcut = 5., framewindow = 6, visual_1cell = False, max_search_radius = 100):
        ...
        mskP = self.cmskSet[0, :, :].copy()
        for im in range(nimg):
            msk = self.cmskSet[im, :, :]
            mskT = self.transform_image(msk, self.cmskSet_t[im, :])
            nc = int(np.max(mskT))
        ...
        linSet = [None]*nimg
        indt0 = np.where(self.cells_indcmskSet == 0)[0]
        ...
        for iS in range(1, nimg):
            ...
            indt1 = np.where(self.cells_indcmskSet == iS)[0]
            xt1 = self.x[indt1, :]
            indt0 = np.where(self.cells_indcmskSet == (iS - 1))[0]
            xt0 = self.x[indt0, :]
            ...
            tracker.volume = ((0, self.cmskSet.shape[2]), (0, self.cmskSet.shape[1]), (-1e5, 1e5)) 
    ...
    # returns indices of interacting partners and shared boundary lengths
    def get_cell_neighborhood(self, indcell, bmsk=None, bunch_clusters=None, rcut=200., visual=False, cpix=5): 
        im = self.cells_indcmskSet[indcell]
        indt = np.where(self.cells_indcmskSet==im)[0]
        ...
        if visual:
            ...
            for ic in range(indcells.size + 1):
                ...
                for ic2 in range(indcells.size + 1):
                    ax.arrow(xset1_im[ic2, 0], xset1_im[ic2, 1], xset1_im[ic2, 0] - xset0_im[ic2, 0], 
                             xset1_im[ic2, 1] - xset0_im[ic2, 1], head_width=10, linewidth=1.5, color='black', alpha=1.0)
                ...
                if ic == 0:
                    ...
                    plt.title('contact {:2.2}, alpha {:2.2}, beta {:2.2} '.format(intersurfaces[ic-1]/np.sum(intersurfaces),
                                                                                  alpha, beta))
                plt.pause(.1)
    ...
    def get_comdx_features(self, cell_inds=None):
        ...
        for im in indimgs:
            ...
            cell_inds_img = np.where(self.cells_indcmskSet[cell_inds] == im)[0]
            indcells,indcomm_cindimg,indcomm_ctraj = np.intersect1d(cell_inds[cell_inds_img],
                                                                    traj_pairSet[:, 1], return_indices=True)
        ...
        for iH in range(1, n_hist + 1):
            ...
            if ended:
                ...
                if indCurrentCell < 0 and not ended:
                    ...
                    indt1 = np.where(self.cells_indcmskSet == indimg1)[0]
                    i1 = np.where(indt1 == indCurrentCell)[0][0]
                    indt0 = np.where(self.cells_indcmskSet == indimg0)[0]
    ...
    def get_unique_trajectories(self, cell_inds=None, verbose=False, extra_depth=None):
        ...
        while n_untracked > 0:
            ...
            if indctracked.size > 0:
                ...
                indComm = indcomm_last + 1 - extra_depth
    ...
    def get_pair_rdf(self, cell_indsA=None, cell_indsB=None, rbins=None, nr=50, rmax=500):
        ...
        indimgsA = np.unique(self.cells_indcmskSet[cell_indsA])
        ...
        for im in indimgs:
            ...
            cell_inds_imgB = np.where(self.cells_indcmskSet[cell_indsB] == im)[0]
    ...
    def get_dx_rdf(self, cell_inds=None, rbins=None, nr=8, rmax=100):
        ...
        for im in indimgs:
            cell_inds_img = np.where(self.cells_indcmskSet[cell_inds] == im)[0]
            indcells,indcomm_cindimg,indcomm_ctraj = np.intersect1d(cell_inds[cell_inds_img], 
                                                                    traj_pairSet[:, 0], return_indices=True)
    ...
    def get_traj_segments(self, seg_length):
        ...
        for itraj in range(ntraj):
            ...
            if traj_len >= seg_length:
                # was -1, think that was an error, changed 2june21 because ended up missing data
    ...
    def get_scaled_sigma(self, neps=100, nr=None): 
        dmat = self.dmat.copy()
    ...
    def get_trajectory_embedding(self, trajl, inds=None, get_trajectories=True, neigen=2,
                                 rcut=5.0, embedding_type='UMAP', n_neighbors=200, min_dist=0.1):
        if inds is None:
    ...
    def explore_2D_img(self,x,cell_traj=None,dm1=None,dm2=None,pathto='./',coordlabel='coord'):
        if self.visual:
            ...
            for ip in range(maxpoints):
                ...
                if ip>0:
                    plt.plot(ptSet[ip-1:ip+1,0],ptSet[ip-1:ip+1,1],'-',linewidth=1,
                             color=plt.cm.jet(1.*ip/maxpoints),alpha=0.5)
                plt.subplot(1,3,2)
                if cell_traj is None:
                    img_pts=plt.imshow(self.X[ind_nn[0],:].reshape(self.maxedge,self.maxedge),
                                       cmap=plt.cm.seismic,clim=(-10,10))
                else:
                    img_pts=plt.imshow(self.X[cell_traj[ip],:].reshape(self.maxedge,self.maxedge),
                                       cmap=plt.cm.seismic,clim=(-10,10))
                plt.axis('off')
                ax=plt.subplot(1,3,3)
                im=self.cells_indcmskSet[cell_traj[ip]]
                indt1=np.where(self.cells_indcmskSet==im)[0]
                msk1=self.cmskSet[im,:,:]
                xt1=self.x[indt1,:]
                contour1_img=plt.contour(xx-self.cmskSet[im,2],yy-self.cmskSet[im,1],
                                         msk1,levels=np.arange(-10,10),cmap=plt.cm.seismic)
            ...
            for it in range(nt - trajl):
                ...
                if is_seq:
                    ...
                    for il in range(trajl):
                         ...
                         cmsky = np.sum(np.multiply(yy, cmskcell))/np.sum(cmskcell)
                         if show_segs:
                             scatter_cc = plt.scatter(np.where(ccborder)[1], np.where(ccborder)[0], s=4,
                                                      c='purple', marker='s', alpha=0.2)
                             scatter_cs = plt.scatter(np.where(csborder)[1], np.where(csborder)[0], s=4,
                                                      c='green', marker='s', alpha=0.2)
                         else:
    ...
    def explore_2D_celltraj_nn(self, x, traj, pts=None, npts=20, dm1=None, dm2=None,
                               pathto='./', coordlabel='coord', show_segs=True):
        if self.visual:
            plt.figure(figsize = (10, 4))
            ipath = 0
            trajl = traj.shape[1]
    ...
    @staticmethod
    def get_stack_trans_frompoints(cmskSet):
        nframes=cmskSet.shape[0]
        tSet=np.zeros((nframes,3))
