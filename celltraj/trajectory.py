...
class cellPoseTraj():
    ...
    def get_image_data(self, n_frame):
        ...
        self.n_frame = n_frame # Number of frames within a file
        nF = self.nF
        timeList = np.array([])
    ...
    def get_cell_neighborhood(self, indcell, bmsk=None, bunch_clusters=None, rcut=200., visual=False, cpix=5): 
        ...
        if visual:
            ...
            for ic in range(indcells.size + 1):
                ...
                plt.imshow(np.ma.masked_where(self.cellborder_omsks[ic].T == 1, self.cellborder_omsks[ic].T), cmap=plt.cm.gray, clim=[-5,5])
                plt.imshow(np.ma.masked_where(self.cellborder_omsks[ic].T == 0, self.cellborder_omsks[ic].T), cmap=plt.cm.seismic, clim=(-5,5))
                imcenter = np.zeros(2)
        ...
        if self.visual:
                ...
                for ic in range(nb*nb):
                    if ic < ncells:
                        ...
                        img_fg = ax[inds2d[0][ic], inds2d[1][ic]].imshow(np.ma.masked_where(cmskcell == 0, cmskcell), cmap=plt.cm.seismic, clim=(-10,10), alpha=1.0)
                        img_bg = ax[inds2d[0][ic], inds2d[1][ic]].imshow(np.ma.masked_where(cmskcell == 1, cmskcell), cmap=plt.cm.gray, clim=(-10,10), alpha=0.6)
    ...
    def explore_2D_img(self,x,cell_traj=None,dm1=None,dm2=None,pathto='./',coordlabel='coord'):
        if self.visual:
            ...
            for ip in range(maxpoints):
                ...
                xt1=self.x[indt1,:]
                contour1_img=plt.contour(xx-self.cmskSet[im,2],yy-self.cmskSet[im,1],msk1,levels=np.arange(-10,10),cmap=plt.cm.seismic)
            ...
            for it in range(nt - trajl):
                ...
                if is_seq:
                    ...
                    for il in range(trajl):
                         ...
                         cmskcell = self.cellborder_cmsks[0]
                         ccborder,csborder = self.get_cc_cs_border(cmskcell)
                         img_fg = plt.imshow(np.ma.masked_where(cmskcell == 0, cmskcell), cmap=plt.cm.seismic, clim=(-10,10), alpha=1.0)
                         img_bg = plt.imshow(np.ma.masked_where(cmskcell == 1, cmskcell), cmap=plt.cm.gray, clim=(-10,10), alpha=0.6)
                         nx = cmskcell.shape[0]; ny = cmskcell.shape[1]
                         xx,yy = np.meshgrid(np.arange(nx), np.arange(ny), indexing='ij')
                         cmskx = np.sum(np.multiply(xx, cmskcell))/np.sum(cmskcell)
                         cmsky = np.sum(np.multiply(yy, cmskcell))/np.sum(cmskcell)
