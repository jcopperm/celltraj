...
class Translate():
	...
	def get_predictedFC(self,state_probs,statesFC):
		...
		for ig in range(nG):
			...
			if np.sum(np.logical_or(np.isnan(v_states),np.isinf(v_states)))==0:
				for itr in range(ntr):
					for istate in range(n):
						x_FC_predicted[itr,ig]=np.sum(np.multiply(state_probs[itr,:],statesFC[:,ig]))
